...
net.Receive( "as_locker_invtolocker", function( _, ply )
	...
	if ent:GetClass() != "as_locker" then ply:ResyncInventory() ent:Resync() return end
	if ent:GetObjectOwner() != ply and not ent.Broken then ply:ResyncInventory() ent:Resync() return end
	if ent:GetProfile() == 0 then ply:ChatPrint("No profile active.") ply:ResyncInventory() ent:Resync() return end
