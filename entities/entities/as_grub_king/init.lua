...
function ENT:Die()
    ...
    owner:RemoveToolFromCache( "tool_grub_king" ) --Remove this object from the player's cache, so it's gone forever.
    ...
    ent:Spawn()
    local amt = ((AS.Items["tool_grub"].craft["misc_chemical"] * 100) * 0.6) + (self:GetInventory()["misc_chemical"] or 0)
    ent:SetAmount( amt )
    ent:SetModelScale( 0.5 + (amt * 0.0005) )
    ent:Activate()
