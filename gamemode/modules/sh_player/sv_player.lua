...
function GM:DoPlayerDeath( ply, attacker, dmg )
    ...
    if attacker:IsPlayer() and ply:IsPlayer() and CurTime() >= (attacker.NextWarRequest or 0) then
        if attacker != ply and attacker:InCommunity() and ply:InCommunity() and ply:GetCommunity() != attacker:GetCommunity() and not ply:IsAllied( attacker:GetCommunity() ) and not ply:IsAtWar( attacker:GetCommunity() ) then
