function PlayerMeta:SetInventory( tbl, noupdate )
    self.Inventory = tbl

    if ( SERVER ) and not noupdate then
...
function PlayerMeta:AddItemToInventory( item, amt, noupdate )
    ...
    self:SetInventory( inv, noupdate )
end

function PlayerMeta:TakeItemFromInventory( item, amt, noupdate )
...
function PlayerMeta:CanUseItem( item )
    ...
    if self:HasAttachment( item ) then self:ChatPrint("You already have this attachment equipped.") return false end
    return true
end
...
function PlayerMeta:GetAllTools() --Will return a table of a player's deployed tools
    ...
    for k, v in pairs( ents.GetAll() ) do
        ...
        if v:GetClass() == "prop_door_rotating" or v:GetClass() == "func_door_rotating" or v:GetClass() == "func_door" then continue end
