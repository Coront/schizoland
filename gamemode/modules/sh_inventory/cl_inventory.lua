...
function AS.Inventory.Open( tab, forcehold )
    ...
    if GetConVar("as_menu_inventory_holdtoopen"):GetInt() > 0 and not forcehold then
        frame_inventory.Think = function( self )
            ...
            and not input.IsButtonDown( input.GetKeyCode( GetConVarString("as_bind_missions") ) ) 
...
--Character Panel
    ...
    local toxic = vgui.Create("DPanel", characterpanel)
    toxic:SetPos( 22, characterpanel:GetTall() - 45)
    toxic:SetSize( 120, 10 )
    function toxic:Paint( w, h )
        local col = COLHUD_BAD:ToTable()
        surface.SetDrawColor( col[1], col[2], col[3], 150 )
        surface.DrawOutlinedRect( 0, 0, w, h, 1 )
        surface.DrawRect( 0, 0, (LocalPlayer():GetToxic() / LocalPlayer():GetMaxToxic()) * w, h )

        surface.SetDrawColor( COLHUD_BAD )
        surface.DrawRect( ((w / LocalPlayer():GetMaxToxic()) * SET.ToxicDebuff), 1, 1, h - 2)
        surface.DrawRect( ((w / LocalPlayer():GetMaxToxic()) * SET.ToxicDebuffHeavy), 1, 1, h - 2)
    end

    local toxicicon = vgui.Create("DPanel", characterdisplay)
    toxicicon:SetPos( 3, characterpanel:GetTall() - 48 )
    toxicicon:SetSize( 16, 16 )
    toxicicon:SetTooltip( "Toxicity" )
    function toxicicon:Paint( w, h )
        surface.SetDrawColor( Color( 0, 175, 25 ) )
        surface.SetMaterial( Material("gui/aftershock/icon/toxin.png") )
        surface.DrawTexturedRect( 0, 0, w, h )
    end

    local toxicamt = vgui.Create("DLabel", characterdisplay)
    toxicamt:SetPos( toxicicon:GetWide() + toxic:GetWide() + 10, characterpanel:GetTall() - 47 )
    toxicamt:SetFont("TargetIDSmall")
    function toxicamt:Think()
        toxicamt:SetText( LocalPlayer():GetToxic() )
        toxicamt:SizeToContents()
    end
...
function AS.Inventory.BuildPlayers()
    ...
    for k, v in pairs( player.GetAll() ) do
        ...
        local name = vgui.Create("DLabel", panel)
        name:SetFont( "TargetID" )
        name:SetText( v:Nickname() )
        name:SetPos( 80, 2 )
        name:SizeToContents()

        local class = vgui.Create("DLabel", panel)
        class:SetFont( "DermaDefault" )
        class:SetText( AS.Classes[v:GetASClass()].name )
        class:SetPos( 80, 18 )
        class:SizeToContents()
        class:SetColor( AS.Classes[v:GetASClass()].color )

        if v:GetCommunityName() != "" then
            local community = vgui.Create("DLabel", panel)
            community:SetFont( "DermaDefault" )
            community:SetText( "Member of " .. v:GetCommunityName() )
            community:SetPos( 80, 33 )
        ...
        if v:GetTitle() != "" then
            ...
            title:SetText( "Title: " .. v:GetTitle() )
            title:SetPos( 80, 48 )
            title:SizeToContents()
        end
